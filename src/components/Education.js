...
function renderProject(project) {
  ...
  const linksHTML = `
    <a href="${project.github}" target="_blank" class="${colors.link} transition-colors" title="View ${project.name} on GitHub">
  ...
  return `
    <div class="group bg-gradient-to-br ${colors.bg} rounded-xl p-6 border border-gray-700/50 ${colors.border} transition-all duration-300">
      <div class="flex items-center justify-between mb-4">
        <div class="text-3xl group-hover:scale-110 transition-transform duration-300">${project.icon}</div>
        <div class="flex gap-2">${linksHTML}</div>
      </div>
      <h5 class="font-medium ${colors.text} mb-2 ${colors.hoverText} transition-colors">${project.name}</h5>
      ...
      <div class="flex flex-wrap gap-2 pt-3 border-t ${colors.borderT}">
        ${project.tags.map((tag) => `<span class="px-2 py-1 ${colors.tag} rounded-md text-xs">${tag}</span>`).join('')}
      </div>
    </div>
  `;
}
...
export function renderEducation() {
  const { university, courses } = educationData;

  const coursesHTML = courses.map((course) => {
    const colorClass = course.color === 'blue' ? 'hover:border-blue-500/30 text-blue-300' :
                       course.color === 'purple' ? 'hover:border-purple-500/30 text-purple-300' :
  ...
  const projectsHTML = projectsData.map((project) => renderProject(project)).join('');
            ...
            <div class="relative bg-gradient-to-br from-gray-800/90 to-slate-800/90 backdrop-blur-sm rounded-3xl p-8 md:p-12 shadow-2xl border border-blue-500/20 hover:border-blue-400/40 transition-all duration-500">
              ...
              <div class="flex flex-col md:flex-row items-center gap-8 mb-12">
                ...
                <div class="text-center md:text-left">
                  ...
                  <p class="text-xl text-gray-300 mb-4">${university.location}</p>
                  <div class="flex flex-wrap gap-4 justify-center md:justify-start">
                    <div class="px-4 py-2 bg-blue-500/20 rounded-xl border border-blue-500/30">
                      <span class="text-blue-300 font-medium">${university.years}</span>
                    </div>
                    <div class="px-4 py-2 bg-purple-500/20 rounded-xl border border-purple-500/30">
                      <span class="text-purple-300 font-medium">CGPA: ${university.cgpa}</span>
                    </div>
                  ...
                  <div class="space-y-6">
                    <div class="bg-gray-800/30 rounded-xl p-6 border border-blue-500/20 hover:border-blue-400/30 transition-all duration-300">
                      <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                          <h5 class="font-semibold text-blue-300">${university.degree}</h5>
                          <p class="text-sm text-gray-400">Graduated with distinction</p>
                        </div>
                      </div>
                      <div class="space-y-4">
                        ${university.achievements.map((achievement, i) => {
                          const gradients = ['from-blue-400 to-purple-400', 'from-purple-400 to-pink-400', 'from-pink-400 to-blue-400'];
                          return `
                            <div class="flex items-start group/item">
                              <div class="w-2 h-2 mt-2 bg-gradient-to-r ${gradients[i % gradients.length]} rounded-full mr-3 group-hover/item:scale-150 transition-transform duration-300"></div>
